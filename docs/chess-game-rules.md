...
- 게임 모드 확장: 2인 대전(PVP) 외에 플레이어가 AI와 대국하는 플레이어 대 컴퓨터(PVC) 모드를 지원한다.
- AI 난이도 레벨 설정 (Rating 300 ~ 2000):
  - **AI-하수 (Rating: 600 ELO)**: 체스 입문자 및 초급자를 위한 난이도 (Depth 1 탐색)
  - **AI-중수 (Rating: 1100 ELO)**: 기본 전술을 구사하는 중급 아마추어 난이도 (Depth 2 탐색)
  - **AI-고급 (Rating: 1600 ELO)**: 클럽 선수 수준의 상급자 난이도 (Depth 2+ 포지션 가치 탐색)
  - **AI-초고수 (Rating: 2000 ELO)**: 마스터/전문가 수준의 고성능 난이도 (Stockfish ELO 2000 튜닝 / Depth 3+ Minimax 탐색)
  - 서버는 플레이어의 턴이 끝난 직후 비동기 스레드 풀(`aiEngineTaskExecutor`)에서 엔진을 호출하여 자동으로 AI의 합법적인 수를 생성하고 적용한다.
