...
- 게임 모드 확장: 2인 대전(PVP) 외에 플레이어가 AI와 대국하는 플레이어 대 컴퓨터(PVC) 모드를 추가한다.
- AI 난이도 설정: AI는 UCI(Universal Chess Interface) 표준 프로토콜을 따르는 외부 체스 엔진(예: Stockfish)을 연동하며, 국제 공인 ELO 레이팅 기준 **최소 2000 수준(Expert/Candidate Master 급)**의 성능을 내도록 깊이(Depth) 또는 엘로 한계(Elo limit)를 설정한다. 서버는 플레이어의 턴이 끝난 직후 백그라운드 스레드에서 엔진을 호출하여 자동으로 AI의 합법적인 수를 생성하고 적용해야 한다.
